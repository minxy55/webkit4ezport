...
QWebPage::WebAction QWebPagePrivate::editorActionForKeyEvent(EZKeyEvent* event)
{
    static struct {
        EZKeySequence::StandardKey standardKey;
        QWebPage::WebAction action;
    } editorActions[] = {
        { EZKeySequence::Cut, QWebPage::Cut },
        { EZKeySequence::Copy, QWebPage::Copy },
        { EZKeySequence::Paste, QWebPage::Paste },
        { EZKeySequence::Undo, QWebPage::Undo },
        { EZKeySequence::Redo, QWebPage::Redo },
        { EZKeySequence::MoveToNextChar, QWebPage::MoveToNextChar },
        { EZKeySequence::MoveToPreviousChar, QWebPage::MoveToPreviousChar },
        { EZKeySequence::MoveToNextWord, QWebPage::MoveToNextWord },
        { EZKeySequence::MoveToPreviousWord, QWebPage::MoveToPreviousWord },
        { EZKeySequence::MoveToNextLine, QWebPage::MoveToNextLine },
        { EZKeySequence::MoveToPreviousLine, QWebPage::MoveToPreviousLine },
        { EZKeySequence::MoveToStartOfLine, QWebPage::MoveToStartOfLine },
        { EZKeySequence::MoveToEndOfLine, QWebPage::MoveToEndOfLine },
        { EZKeySequence::MoveToStartOfBlock, QWebPage::MoveToStartOfBlock },
        { EZKeySequence::MoveToEndOfBlock, QWebPage::MoveToEndOfBlock },
        { EZKeySequence::MoveToStartOfDocument, QWebPage::MoveToStartOfDocument },
        { EZKeySequence::MoveToEndOfDocument, QWebPage::MoveToEndOfDocument },
        { EZKeySequence::SelectNextChar, QWebPage::SelectNextChar },
        { EZKeySequence::SelectPreviousChar, QWebPage::SelectPreviousChar },
        { EZKeySequence::SelectNextWord, QWebPage::SelectNextWord },
        { EZKeySequence::SelectPreviousWord, QWebPage::SelectPreviousWord },
        { EZKeySequence::SelectNextLine, QWebPage::SelectNextLine },
        { EZKeySequence::SelectPreviousLine, QWebPage::SelectPreviousLine },
        { EZKeySequence::SelectStartOfLine, QWebPage::SelectStartOfLine },
        { EZKeySequence::SelectEndOfLine, QWebPage::SelectEndOfLine },
        { EZKeySequence::SelectStartOfBlock, QWebPage::SelectStartOfBlock },
        { EZKeySequence::SelectEndOfBlock,  QWebPage::SelectEndOfBlock },
        { EZKeySequence::SelectStartOfDocument, QWebPage::SelectStartOfDocument },
        { EZKeySequence::SelectEndOfDocument, QWebPage::SelectEndOfDocument },
        { EZKeySequence::DeleteStartOfWord, QWebPage::DeleteStartOfWord },
        { EZKeySequence::DeleteEndOfWord, QWebPage::DeleteEndOfWord },
        { EZKeySequence::InsertParagraphSeparator, QWebPage::InsertParagraphSeparator },
        { EZKeySequence::InsertLineSeparator, QWebPage::InsertLineSeparator },
        { EZKeySequence::SelectAll, QWebPage::SelectAll },
        { EZKeySequence::UnknownKey, QWebPage::NoWebAction }
    };

    for (int i = 0; editorActions[i].standardKey != EZKeySequence::UnknownKey; ++i)
        if (event->keySequence() == editorActions[i].standardKey)
...
void QWebPagePrivate::keyPressEvent(EZKeyEvent *ev)
{
    bool handled = false;
    ...
    if (!handled) {
        ...
        if (!handleScrolling(ev, frame)) {
            ...
            case EZ::Key_Backspace:
                if (ev->modifiers() == EZ::ShiftModifier)
                    q->triggerAction(QWebPage::Forward);
                else
                    q->triggerAction(QWebPage::Back);
                break;
...
void QWebPagePrivate::keyReleaseEvent(EZKeyEvent *ev)
{
    if (ev->isAutoRepeat()) {
        ev->setAccepted(true);
        return;
...
void QWebPagePrivate::shortcutOverrideEvent(EZKeyEvent* event)
{
#ifdef EZ_PORT
    WebCore::Frame* frame = page->focusController()->focusedOrMainFrame();
    WebCore::Editor* editor = frame->editor();
...
bool QWebPagePrivate::handleScrolling(EZKeyEvent *ev, Frame *frame)
{
    ScrollDirection direction;
    ScrollGranularity granularity;

#ifndef QT_NO_SHORTCUT
    if (ev->keySequence() == EZKeySequence::MoveToNextPage
        || (ev->key() == EZ::Key_Space && !(ev->modifiers() & EZ::ShiftModifier))) {
        granularity = ScrollByPage;
        direction = ScrollDown;
    } else if (ev->keySequence() == EZKeySequence::MoveToPreviousPage
               || ((ev->key() == EZ::Key_Space) && (ev->modifiers() & EZ::ShiftModifier))) {
        granularity = ScrollByPage;
        direction = ScrollUp;
    } else
#endif // QT_NO_SHORTCUT
    if ((ev->key() == EZ::Key_Up && ev->modifiers() & EZ::ControlModifier)
               || ev->key() == EZ::Key_Home) {
        granularity = ScrollByDocument;
        direction = ScrollUp;
    } else if ((ev->key() == EZ::Key_Down && ev->modifiers() & EZ::ControlModifier)
               || ev->key() == EZ::Key_End) {
        granularity = ScrollByDocument;
        direction = ScrollDown;
    } else {
...
bool QWebPage::event(QEvent *ev)
{
    ...
    case QEvent::KeyPress:
        d->keyPressEvent(static_cast<EZKeyEvent*>(ev));
        break;
    case QEvent::KeyRelease:
        d->keyReleaseEvent(static_cast<EZKeyEvent*>(ev));
        break;
    case QEvent::FocusIn:
...
{
    EZKeyEvent ev(EZKeyEvent::KeyPress, EZ::Key_Tab, EZKeyEvent::KeyboardModifiers(next ? EZ::NoModifier : EZ::ShiftModifier));
    d->keyPressEvent(&ev);
