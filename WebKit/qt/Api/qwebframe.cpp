...
//Cross Dom start ;add by maorui
void QWebFrame::addToJavaScriptDomWindow(const QString &name, QWebFrame *that)
{
    if (!page()->settings()->testAttribute(QWebSettings::JavascriptEnabled))
        return;
    JSC::JSLock lock(JSC::SilenceAssertionsOnly);
    JSDOMWindow* window = toJSDOMWindow(d->frame, mainThreadNormalWorld());
    JSDOMWindow* thatWin = toJSDOMWindow(that->d->frame, mainThreadNormalWorld());

    JSC::ExecState* exec = window->globalExec();
    JSC::PutPropertySlot slot;
    window->put(exec, JSC::Identifier(exec, reinterpret_cast_ptr<const UChar*>(name.constData()), name.length()), thatWin, slot);
}
// Cross Dom end
